...
def analyze_plant_dataset(dataset_path):
    ...
    :param dataset_path: Full path to the plant directory (e.g., ./dataset/apple).
    """
    ...
    for disease in os.listdir(dataset_path):
        ...
        if os.path.isdir(disease_path):
            count = len([img for img in os.listdir(disease_path) if os.path.isfile(os.path.join(disease_path, img))])
            category_counts[disease] = count

    if not category_counts:
        print(f"Error: No categories found in '{dataset_path}'. Make sure it contains subdirectories with images.")
        sys.exit(1)
